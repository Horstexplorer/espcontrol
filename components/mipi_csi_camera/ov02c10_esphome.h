...
// Small C++ adapter around the vendored, non-`esp_cam_sensor`-managed OV02C10 driver
// (`ov02c10.c`/`ov02c10*.h`). This keeps every OV02C10-specific detail - the linker
// keep-alive workaround and the fixed-gain color correction the ISP's uncorrected demosaic needs
// for this sensor - out of the generic `mipi_csi_camera.cpp`/`.h`, so adding another vendored
// sensor later only means adding another `<name>_esphome.{h,cpp}` adapter with the same shape,
// not touching the generic driver code.
//
...
/// reverted). `num_pixels` is the number of 16-bit RGB565 pixels in `pixels`, not the byte count.
